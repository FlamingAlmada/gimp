...
typedef struct _GimpGridPrivate GimpGridPrivate;

struct _GimpGridPrivate
{
  gboolean grid_visible;
};

#define GET_PRIVATE(item) G_TYPE_INSTANCE_GET_PRIVATE (\
	item, GIMP_TYPE_GRID, GimpGridPrivate\
)
...
static void gimp_grid_class_init (GimpGridClass *klass)
{
  ...
  GIMP_CONFIG_INSTALL_PROP_UNIT (object_class, PROP_OFFSET_UNIT,
	...
	GIMP_CONFIG_INSTALL_PROP_BOOLEAN (object_class, PROP_GRID_VISIBLE,
		"grid-visible", NULL,
		FALSE,
		GIMP_PARAM_STATIC_STRINGS
	);
    ...
    case PROP_OFFSET_UNIT:
      ...
      break;
		case PROP_GRID_VISIBLE:
			g_value_set_boolean(value, grid->grid_visible);
			break;
    default:
                        ...
                        GParamSpec   *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_OFFSET_UNIT:
      ...
      break;
		case PROP_GRID_VISIBLE:
			grid->grid_visible = g_value_get_boolean(value);
			break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
      break;
...
gboolean
gimp_grid_get_visible (	GimpGrid *grid )
{
  g_return_val_if_fail (GIMP_IS_GRID (grid), FALSE);

  return GET_PRIVATE( grid )->grid_visible;
}

// ---------------- SETTERS

void gimp_grid_set_visible (	GimpGrid *grid, gboolean grid_visible )
{
  g_return_if_fail (GIMP_IS_GRID (grid));

	if( gimp_grid_get_visible( grid ) != grid_visible )
	{
		GET_PRIVATE( grid )->grid_visible = grid_visible;
	}
}
